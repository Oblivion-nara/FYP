...
public class ASearch {
	...
	private ArrayList<PointHeuristic> toExplore;
	private Random rand;
	...
	private void Search() {
		...
		while (!goalReached) {
			...
			System.out.println("finding moves "+i);
		}
	...
	private ArrayList<PointHeuristic> getChildren(PointHeuristic parent) {
		...
		for (int x = -movement; x <= movement; x += 10) {
			for (int y = -movement; y <= movement; y += 10) {
				...
				if (rand.nextFloat() < 0.05 && !children.isEmpty()) {
					continue;
				}
				children.add(point);
			}
	...
	// private void Search() {
	// PointHeuristic current = new PointHeuristic(0, track.getStart(), new
	// Point(0, 0), 0, null);
	// boolean goalReached = false;
	// toExplore.add(current);
	// int i = 0;
	...
	// continue;
	// }
	// Point location = new Point(parent.getLocation().x +
	// parent.getVelocity().x + x,
	// parent.getLocation().y + parent.getVelocity().y + y);
	// if (!track.onTrack(location)) {
	// continue;
	// }
	...
	// continue;
	// }
	// if (track.checkMove((Point) location, (Point) parent.getLocation()) !=
	// null) {
	// continue;
	// }
	//
